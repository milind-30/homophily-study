...
jatos_data_path1 = "data/raw/prolific_first_pass.jsonl"
jatos_data_path2 = "data/raw/prolific_second_pass.jsonl"
data1 = helpers.load_jsonl(jatos_data_path1)
data2 = helpers.load_jsonl(jatos_data_path2)
...
for data_index, data in enumerate(datasets):
    conditions = helpers.retrieve_conditions(data)
    task_orderings = helpers.retrieve_task_ordering(data)
    subject_demographics = helpers.retrieve_subj_demographics(data)
    subject_selfratings = helpers.retrieve_confidence(data)
    subject_deceptions = helpers.retrieve_deception(data)
    ...
    for i, participant_data in enumerate(data):
        ...
        for entry in participant_data:
            ...
            merged["subject_ability"].append(subject_selfratings[i][0])
            merged["subject_awareness"].append(subject_deception)
            merged["rt"].append(rt)
...
outpath = "data/preprocessed/prolific_pairwise.csv"
